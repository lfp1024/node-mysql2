...
function readCodeFor(type, charset, encodingExpr, config, options) {
  ...
  switch (type) {
    ...
    case Types.DATETIME:
      console.log('[mysql2]==========DATETIME=======', type);
    // eslint-disable-next-line no-fallthrough
    case Types.TIMESTAMP:
      console.log(
        '[mysql2]==========DATETIME >> typeMatch=======',
        helpers.typeMatch(type, dateStrings, Types)
      );
      console.log(
        '[mysql2]==========DATETIME >> dateStrings=======',
        dateStrings
      );
      if (helpers.typeMatch(type, dateStrings, Types)) {
        return 'packet.readLengthCodedString("ascii")';
      }
      console.log('[mysql2]==========DATETIME >> return======= parseDateTime');
...
function compile(fields, options, config) {
  ...
  for (i = 0; i < fields.length; i++) {
    ...
    console.log('[mysql2 ======text_parser readCode======= ]', readCode);
    console.log(
      '[mysql2 ======text_parser typeCast======= ]',
      options.typeCast
    );
    if (typeof options.typeCast === 'function') {
      // 自定义转换规则
      parserFn(
        `${lvalue} = options.typeCast(
          wrap(fields[${i}], ${helpers.srcEscape(
          typeNames[fields[i].columnType]
        )}, packet, ${encodingExpr}), 
          function() { return ${readCode};}
          )`
      );
      ...
      console.log('走原始的typecast ', readCode);
      parserFn(`${lvalue} = ${readCode};`); // 转换
    }
  }
