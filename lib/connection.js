...
class Connection extends EventEmitter {
  constructor(opts) {
    super();
    console.log('[mysql2]======timezone====== ', opts.config.timezone);
    console.log('[mysql2]======dateStrings=== ', opts.config.dateStrings);
    // console.log('[mysql2]=======typeCast===== ', Function.prototype.toString.call(opts.config.typeCast));
    console.log('[mysql2]=======typeCast===== ', opts.config.typeCast);
  ...
  handlePacket(packet) {
    console.log('[mysql2] >>>>> handlePacket packet = ', packet);
    if (this._paused) {
    ...
    const done = this._command.execute(packet, this); // command.js 中实现的 execute 方法
    console.log('[mysql2] >>>>> handlePacket done = ', done);
    if (done) {
  ...
  addCommand(cmd) {
    ...
    console.log('[mysql2] >>>>> this._command', this._command);
    if (!this._command) {
      this._command = cmd;
      this.handlePacket();
    } else {
  ...
  execute(sql, values, cb) {
    let options = {};
    console.log('[mysql2] >>>>> typeof  sql = ', typeof sql); // string
    console.log('[mysql2] >>>>> typeof values = ', typeof values); // object
    if (typeof sql === 'object') {
      // execute(options, cb)
      options = sql;
      ...
      // execute(sql, values, cb)
      console.log('[mysql2] >>>>> sql = ', sql);
      console.log('[mysql2] >>>>> values = ', values);
      options.sql = sql;
